...
const ElectricBorder: React.FC<ElectricBorderProps> = ({
  ...
  const updateAnim = () => {
    ...
    requestAnimationFrame(() => {
      const anims: NodeListOf<SVGAnimateElement> = svg.querySelectorAll(
        'feOffset > animate[attributeName="dy"], feOffset > animate[attributeName="dx"]'
      );
      anims.forEach((a) => {
  ...
  return (
    <div
      ref={rootRef}
      className={`${styles["electric-border"]} ${className ?? ""}`}
      style={{ ...vars, ...style }}
    >
      <svg ref={svgRef} className={styles["eb-svg"]} aria-hidden focusable="false">
        <defs>
          <filter
            id={filterId}
      ...
      <div className={styles["eb-layers"]}>
        <div ref={strokeRef} className={styles["eb-stroke"]} />
